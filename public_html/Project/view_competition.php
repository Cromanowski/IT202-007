...
//handle page load
$stmt = $db->prepare("SELECT Competitions.id, title, min_participants, current_participants, current_reward, expires, creator_id, min_score, join_cost, IF(competition_id is null, 0, 1) as joined,  CONCAT(first_place,'% - ', second_place, '% - ', third_place, '%') as place FROM BGD_Competitions
JOIN Payout_Options on Payout_Options.id = BGD_Competitions.payout_option
LEFT JOIN BGD_UserComps on UserComps.competition_id = BGD_Competitions.id WHERE user_id = :uid AND BGD_Competitions.id = :cid");
$row = [];
$comp = "";
try {
