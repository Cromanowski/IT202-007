...
{
    $db = getDB();
    $stmt = $db->prepare("SELECT * FROM (SELECT s.user_id, s.score, s.created, a.username, DENSE_RANK() OVER (PARTITION BY s.user_id ORDER BY s.score desc) as `rank` FROM Scores s
